...
model_save_dir = "models_{}_1DVAE".format(modality)
if not os.path.isdir(model_save_dir):
    os.mkdir(model_save_dir)
results_save_dir = "results_{}_1DVAE".format(modality)
...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.efc1  = nn.Linear(h_layer_2 * (sub_window_size-6), h_layer_5)
        ...
        self.dfc2 = nn.Linear(h_layer_5, h_layer_2 * (sub_window_size-6))
        self.ddrop1 = nn.Dropout(p=0.3, inplace = False)
        self.dconv3 = nn.ConvTranspose1d(h_layer_2, h_layer_1, kernel_size=kernel_size, stride=stride, padding = 0, output_padding = 0)
        self.dbn3 = nn.BatchNorm1d(h_layer_1, eps = 1e-5, momentum = 0.1, affine = True, track_running_stats = True)
        self.dconv4 = nn.ConvTranspose1d(h_layer_1, n_channels, kernel_size=kernel_size, padding = 0, stride=stride)
        #
    ...
    def Encoder(self, x):
        ...
        #print(eh2.shape)
        eh5 = self.relu(self.edrop1(self.efc1(eh2.view(-1, h_layer_2 * (sub_window_size-6)))))
        mu_z = self.mu_z(eh5)
    ...
    def Decoder(self, z):
        ...
        #print(dh2.shape)
        dh5 = self.relu(self.dbn3(self.dconv3(dh2.view(-1, h_layer_2, (sub_window_size-6)))))
...
all_normal_imgs = np.array([(img - np.min(img)) / (np.max(img) - np.min(img))
                       for img in all_normal_prep_eegs])  # batch x 1 x channels x time points
...
if Restore:
    ...
    test_seizure_imgs = np.array([(img - np.min(img)) / (np.max(img) - np.min(img))
                                  for img in test_seizure_prep_eegs])  # batch x 1 x channels x time points
